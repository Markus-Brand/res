...
public class Grid implements Iterable<Tile> {
    ...
    public Grid(GridMode mode, TileType defaultTileType) {
        this(mode, defaultTileType, new Point(1, 1));
    }

    public Grid(GridMode mode, TileType defaultTileType, Point startSize) {
    ...
    public List<Point> getNeighbours(Point coords) {
        List<Point> l = new ArrayList<>(mode.getNeighbourCount());
        if (mode == GridMode.Grid4) {
    ...
    private boolean containsCoord(Point p) {
        ...
        System.err.println("missing here: containsCoord " + mode);
        return false;
